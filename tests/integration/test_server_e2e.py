...
def test_chat_stream(server_fixture: subprocess.Popen[str]) -> None:
    ...
    # Check for valid structured output in the response
    has_output_content = False
    for event in events:
        output = event.get("output")
        if output is not None and "decision" in output:
            has_output_content = True
            assert output["decision"] == "approved"
            assert output["reviewer"] == "System (Auto-approve)"
            break

    assert has_output_content, (
        "Expected at least one event with structured output content"
    )
